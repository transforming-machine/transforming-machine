...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Transforming-Machine thinks for you about the meanings.",
        epilog="Example:  transforming-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | transforming-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | transforming-machine         #

    $ transforming-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="transforming-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"transforming-machine: {utterance}.")
        syslog.closelog()
